...
<script>
	function display(trno)
	{
		...
		if(status)
		{
			row = document.getElementById(trno); // row --> <tr>
			td	= row.getElementsByTagName("td"); // td ---><td> : td[0],td[1],td[2]
			trno =  td[0].textContent;
			tname = td[1].querySelector("input").value;
			tper =  td[2].querySelector("input").value;
			
			
			// now I am calling UpdateServlet using POST method
			// and adding 'rno', 'name', 'per' in request object
			
			fetch('http://localhost:8080/APP-SMS/UpdateStudent', { 
					method: 'POST',	
